...
function App() {
  ...
  const connectToServer = useCallback(async () => {
    ...
    try {
      ...
      const socket = createSocket(client, false, false);
      ...
      socket.onmatchdata = (matchData) => {
        ...
        switch (matchData.op_code) {
          ...
          case OpCode.GAME_OVER: {
            ...
            const board = Array.isArray(data.board) ? data.board : Array.isArray(data.finalBoard) ? data.finalBoard : null;
            const rawWinner = typeof data.winner === 'string' ? data.winner : null;
            const winnerIdFromPayload = typeof data.winnerId === 'string'
              ? data.winnerId
              : (rawWinner && rawWinner !== 'draw'
                ? rawWinner
                : null);
            const latestState = gameStateRef.current;
            const winnerName = !isDraw && winnerIdFromPayload && latestState?.players?.[winnerIdFromPayload]
          ...
          case OpCode.ERROR: {
            ...
            if (data.error && data.error.toLowerCase().includes('not your turn')) {
              setErrorMessage(null);
            } else if (data.error && (data.error.toLowerCase().includes("player 1's turn") || data.error.toLowerCase().includes("player 2's turn"))) {
              setErrorMessage(null);
            } else {
      ...
      <div className="min-h-screen p-4 pb-8">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            ...
            <ScorePanel title="Moves" value={gameState?.moveCount || 0} highlight={isMyTurn()} />
            <ScorePanel 
              title="Status" 
              value={gameState?.gameStatus === 'waiting' ? 'Wait' : (gameState?.gameStatus === 'playing' || gameState?.gameStatus === 'active') ? 'Play' : 'End'} 
            />
          </div>
          ...
          {(gameState?.gameStatus === 'waiting' || playerCount < 2) && (
            <div className="bg-tile-x bg-opacity-20 border-2 border-tile-x rounded-lg p-4 mb-6 text-center animate-pulse">
              <p className="font-bold text-text-dark">Waiting for opponent to join...</p>
            </div>
          )}

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <PlayerInfo player={myPlayer} isCurrentTurn={isMyTurn()} userId={userId} />
            <PlayerInfo player={opponentPlayer} isCurrentTurn={!isMyTurn() && gameState?.gameStatus === 'playing'} userId={userId} />
          </div>
          ...
          {gameState && (
            <Board
              ...
              onMove={handleMove}
              disabled={!isMyTurn() || (gameState.gameStatus !== 'playing' && gameState.gameStatus !== 'active')}
  ...
  return (
    <div className="min-h-screen bg-game-bg">
      ...
      {(connectionState === 'connected' || connectionState === 'finding-match') && renderLobbyScreen()}
