...
function processMove(message, state, dispatcher, logger, nk) {
...
function broadcastState(dispatcher, state, logger) {
    ...
    const stateMessage = JSON.stringify({
        ...
        currentPlayerName: ((_a = state.players[state.currentPlayer]) === null || _a === void 0 ? void 0 : _a.username) || '',
        gameStatus: state.gameStatus,
...
function sendError(dispatcher, presence, errorMessage) {
    const message = JSON.stringify({
        type: 'error',
        error: errorMessage,
    });
...
function updateLeaderboard(nk, logger, userId, username, won) {
    try {
        const score = won ? 1 : 0;
...
const initLeaderboardRpc = function (ctx, logger, nk, payload) {
    try {
        initLeaderboard(nk, logger);
